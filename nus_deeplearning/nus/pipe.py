...
def create_dic(udic, datetimeobj=datetime.datetime.now()):
    ...
    if udic[0]["encoding"] == "tppi":
        ...
        udic[0]["encoding"] == "complex"
        or udic[0]["encoding"] == "states"
        or udic[0]["encoding"] == "states-tppi"
    ):
...
def add_axis_to_dic(dic, adic, n):
    ...
    if n == 0 or dic["FD2DPHASE"] != 1:
        ...
        dic[fn + "CENTER"] = int(psize / 2.0) + 1.0

    else:  # TPPI requires division by 4

        dic[fn + "CENTER"] = int(psize / 4.0) + 1
    ...
    dic[fn + "ORIG"] = (
        dic[fn + "CAR"] * dic[fn + "OBS"]
        - dic[fn + "SW"] * (osize - dic[fn + "CENTER"]) / osize
    )
...
def pack_complex(data):
    ...
    return np.array(data[..., ::2] + data[..., 1::2] * 1.0j, dtype="complex64")
...
def dic2fdata(dic):
    ...
    fdata[16:18] = struct.unpack("2f", struct.pack("8s", dic["FDF2LABEL"].encode()))

    fdata[18:20] = struct.unpack("2f", struct.pack("8s", dic["FDF1LABEL"].encode()))

    fdata[20:22] = struct.unpack("2f", struct.pack("8s", dic["FDF3LABEL"].encode()))

    fdata[22:24] = struct.unpack("2f", struct.pack("8s", dic["FDF4LABEL"].encode()))

    # and the longer strings (typically blank)

    fdata[286:290] = struct.unpack("4f", struct.pack("16s", dic["FDSRCNAME"].encode()))

    fdata[290:294] = struct.unpack("4f", struct.pack("16s", dic["FDUSERNAME"].encode()))

    fdata[297:312] = struct.unpack("15f", struct.pack("60s", dic["FDTITLE"].encode()))

    fdata[312:352] = struct.unpack(
        "40f", struct.pack("160s", dic["FDCOMMENT"].encode())
    )

    fdata[464:472] = struct.unpack("8f", struct.pack("32s", dic["FDOPERNAME"].encode()))
